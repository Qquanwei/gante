...
export default React.memo<IPinProps>(function Pin({
  className,
  pin,
  dragMode,
  showPin,
  ...
  const onDragPinStart = useCallback(
    (event) => {
      event.dataTransfer.setData(
        "text/plain",
        JSON.stringify({ type: "pin", pinIdx: data ? data.pinIdx : -1 }),
      );
    ...
    <Popup
      ...
      previewContent={
        <div
          ...
          className="hover:z-10 break-all font-normal transition-all duration-75 text-left select-text relative hover:bg-yellow-100/90 bg-yellow-100/60 p-2 rounded min-w-[100px] min-h-[50px]"
        >
          {data?.content}
        </div>
      }
      content={({ close }) => (
        <div>
          <form ref={formRef}>
            <div className="flex items-center">
              ...
              <input
                ...
                type="checkbox"
                value="enable"
                defaultChecked={data?.fixed === "enable"}
              />
            </div>
            <textarea
              name="content"
          ...
          <div className="flex justify-between">
            <Button
              className="w-[90px] border rounded flex justify-center items-center bg-rose-300 text-white"
              onClick={onClickDelete}
            >
              删除
            </Button>
            <Button
              className="w-[100px] border rounded flex justify-center items-center bg-white"
              onClick={onClickSave(close)}
            >
              保存
            </Button>
          </div>
    ...
    >
      <div
        ...
        className={classNames(
          "cursor-pointer w-[20px] bg-transparent h-[20px] bg-[url(/tuding.png)] bg-contain",
          className,
          {
