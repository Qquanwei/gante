    ...
    <div>
      <div
        ...
        classNames("flex justify-center items-center text-xs select-none cursor-pointer w-[50px] h-[50px]  rounded-full", {
          'animate-bounce animate-[bounce_.8s_3_linear]': (isLogin && user)? !user.is_contributor : false
        })
        }>
        {
          isLogin ? (
            <Menu menuButton={<MenuButton><img src={ (user && user.avatar) || '/user.png'} className="rounded-full bg-black/5 border-box hover:border-sky-500/40 border-2 border-sky-500/10" width="50" height="50" /></MenuButton>} transition>
              <Link href='/user'>
                <MenuItem>
                  个人中心
                </MenuItem>
      ...
      <Modal show={showLoginModal} onClose={onModalClose} title="登录授权">
        <div className="flex items-center justify-center flex-col mx-auto mt-[20px] w-[300px]">
          ...
          <a className="hidden github-login block border flex items-center justify-center hover:bg-gray-300 w-full mt-[10px] h-[60px]" noreferer="true" href={`https://github.com/login/oauth/authorize?login&client_id=${process.env.GANTE_GITHUB_CLIENT_ID}&scope=user`} >
